...
function twentyfifteen_setup() {
	...
	register_nav_menus( array(
		...
		'top_header' => __( 'Top Header Menu',      'twentyfifteen' ),
...
/* Theme action and filter functions */
...
/* PayPal Pro */
require get_template_directory() . '/paypal/paypal-pro.php';
...
function cart_order_update(){
    //session_start();
    $session_id = session_id();
    global $wpdb;
    $tbl_cart = $wpdb->prefix.'cart';
    $query = "SELECT * FROM $tbl_cart WHERE id_cart = (SELECT MAX(id_cart) FROM $tbl_cart WHERE id_session = '$session_id')";
    $result = $wpdb->get_row($query);
    if($result <> NULL){
        if($result->is_order == 1){
            if(isset($_SESSION['cart'])){
                unset($_SESSION['cart']);
                unset($_SESSION['combo']);
                unset($_SESSION['pickup_time']);
                unset($_SESSION['rege_no']);
                unset($_SESSION['burger_note']);
                unset($_SESSION['cus_note']);
...
function test_time_slot(){
    $starttime = '11:00';  // your start time
